...
function BomPage() {
    ...
    // 載入 Matrix 資料
    useEffect(() => {
        if (selectedRevisionId && isMatrixMode) {
            fetchMatrixData(selectedRevisionId)
        }
    }, [selectedRevisionId, isMatrixMode, fetchMatrixData])
    ...
    // 切換 Matrix 模式時，若不在 All View 建議切換?
    // 暫時保持現狀，讓使用者自行決定 Filter
                        ...
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium rounded-lg transition-colors border
                                ${isMatrixMode
                                    ...
                                    : 'bg-white text-slate-600 border-slate-200 dark:bg-surface-800 dark:text-slate-300 dark:border-slate-700 hover:bg-slate-50'
                                }`}
                            title="切換 Matrix BOM 模式"
                        >
                            <Grid3X3 size={14} />
                            {isMatrixMode ? 'Matrix' : 'Standard'}
                        </button>
