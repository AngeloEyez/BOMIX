...
永遠使用繁體中文回覆，包含程式碼註解、Commit 訊息、任務規劃、任務執行報告、提出 PR 說明、PR 回應、與使用者對話等所有產出物。此規則為最高優先級，確保每次都能被嚴格遵守，包含切換模型或重開對話。
...
### 每次任務開始前，必須按以下順序閱讀文件：
1. **`dev/lessonlearned.md`** — 了解系統性錯誤與歷史經驗，避免重複踩坑
2. **`dev/PLAN.md`** — 確認當前開發階段與待辦事項
3. **`dev/SPEC.md`** — 了解功能需求與資料結構定義
4. **`dev/ARCHITECTURE.md`** — 了解系統架構
5. **`.gemini/GEMINI.md`** — 了解開發規範（即本文件）
6. **`dev/DATABASE.md`** — 了解資料庫 Schema
7. **`dev/COLLABORATION.md`** — 了解 Jules 與 Antigravity 的協作規範
...
- **維護 Lesson Learned**：遇到系統性錯誤並解決後，若判定日後會重複遇到，必須將問題與解法記錄至 `dev/lessonlearned.md`，避免重複嘗試浪費 Token
