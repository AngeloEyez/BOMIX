...
│   │   ├── SeriesPage                  ← 系列屬性管理
│   │   ├── ProjectPage                 ← 專案管理
│   │   ├── BomPage                     ← BOM 檢視/編輯/Matrix (策略模式架構)
│   │   ├── ComparePage                 ← 版本比較
│   │   └── SettingsPage                ← 系統設定
│   └── StatusBar                       ← 底部狀態列 (包含 Task/System 日誌)
...
### 導航機制
- 原本由左側 Sidebar 導航，已改為頂部 Header 導航列。
- 使用 Zustand (`useNavStore`) 控制全局頁面切換 (`currentPage`)。
- 頁面以條件渲染方式動態顯示（避免重建不相關的元件，但確保切換時狀態保留或適當卸載）。

### BomPage 頁面架構
- 針對 `BomPage` 單一頁面過大、難以維護的問題，已進行重構。
- 採用 **策略模式 (Strategy Pattern)** 與 **複合元件 (Compound Components)**。
- 將視圖邏輯拆分為 `StandardBomView.jsx`、`MatrixBomView.jsx`，並由通用的 `BomToolbar.jsx` 控制狀態。
- 核心資料表保留在 `BomTable.jsx`，以純 UI 元件的身份接收 `columns` 與 `data`。
- 詳細架構請參閱 [BomPage 架構說明](codeStructure/BomPage.md)。
...
- 應用程式設定（視窗大小、主題偏好、自動載入上次專案等）存於使用者目錄。
- 使用 electron-store 或自定義 JSON 格式儲存（`settings.json`）。
- 新增了 `SettingsPage` UI 讓使用者可以直接在應用程式內修改這些偏好設定。
- 詳細 API 與設定選項請參閱 [SETTINGS_GUIDE.md](codeStructure/SETTINGS_GUIDE.md)。
...
詳細 API 說明請參閱 [LOG_SYSTEM.md](codeStructure/LOG_SYSTEM.md)
