...
function getWorkspaceVisibleWidth(): number {
  ...
  // VirtualScroller 的 clientWidth 已自動扣除垂直 scrollbar，為最精準的量測基準
  if (wrapperEl) {
    ...
    // 回退：直接讀取容器寬度，並扣除垂直 scrollbar 估算值 (17px)
  ...
  // 最後回退：粗估值
  return Math.max(Math.floor(window.innerWidth * 0.75), 400)
}
...
// ── 拖曳選取自動平移滾動引擎 (Selection Drag Auto-Scroll Engine) ────
 ...
 * @param node 欲搜尋的 DOM 節點
 * @returns 最末端的 Text Node，若不存在則回傳 null
 */
...
/* 強制表格遵守設定欄寬，徹底防止長文字欄位 (Description / Location) 將單元格無限撐開 */
...
/* 儲存格文字容器：單行平時裁切、支援鼠標向右拖曳自動滾動選取完整內容 */
.cell-text {
  display: block;
  width: 100%;
  max-width: 100%;
...
/* 拖曳選取、雙擊選取、全選或聚焦期間：立即消除省略號，完整顯露末端真實文字 */
...
/* 徹底隱藏 Chrome / Edge / WebView2 原生水平捲軸，保持 26px 緊湊美觀 */
