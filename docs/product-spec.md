...
> **⚠️ Wails v3 開發注意事項：**
> 開發時（特別是 Claude Code 或其他 AI Agent）須注意，Wails v3 的官方 API 與 v2 **完全不相容**。前端綁定與事件監聽需使用 v3 的呼叫方式（例如透過 `@wailsio/runtime`），後端事件派發 (Event Emission) 亦需使用 Wails v3 Application Instance 的 Event API，切勿混用 v2 的語法。
