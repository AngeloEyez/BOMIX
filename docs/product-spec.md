...
| **路由管理** | Vue Router | 4.x | 處理不同頁面的動態切換 |
| **狀態管理** | Pinia | 2.x | 處理跨元件的任務狀態與資料庫連線狀態共享 |
...
> **⚠️ Wails v3 開發注意事項：**
> 
> Wails v3 的官方 API 與 v2 **完全不相容**。開發時（特別是 Claude Code 或其他 AI Agent）須嚴格遵守以下規範：
> 
> ### 後端 API 差異
> 
> | 功能 | Wails v2 (❌ 錯誤) | Wails v3 (✅ 正確) |
> |------|-------------------|-------------------|
> | 引入路徑 | `"github.com/wailsapp/wails/v2/pkg/runtime"` | `"github.com/wailsapp/wails/v3/pkg/application"` |
> | 事件發射 | `runtime.EventsEmit(ctx, eventName, data)` | `app.Event.Emit(eventName, data)` |
> | 應用程式建構 | `runtime.New()` | `application.New(application.Options{...})` |
> | Service 註冊 | `app.Bind()` | `app.RegisterService(application.NewService(instance))` |
> 
> ### 事件系統
> 
> - **v2**: 使用 `context.Context` 傳遞 runtime，透過 `runtime.EventsEmit(ctx, ...)` 發射事件
> - **v3**: 直接使用 `application.App` 實例的 `Event.Emit()` 方法，無需 context
> 
> ### 正確的後端初始化模式 (v3)
> 
> ```go
...
> 
> type App struct {
>     app    *application.App  // 儲存 application 實例引用
...
> ### 編譯與建置指令
> 
> Wails v3 使用純 Go 實現，Windows 版本直接透過 syscall 呼叫 Windows 原生 API 與 WebView2，**不需要 CGO**。
> 
> **開發模式：**
> ```bash
> cd bomix-app
...
#### 3.3.3 SQLite 併發與寫入優化 (Database is locked 防範)

SQLite 對於並發寫入 (Concurrent Writes) 支援有限，若多個匯入任務同時寫入，極易觸發 `database is locked` 錯誤。為確保系統穩定性，資料庫連線與寫入必須嚴格遵循以下規範：

1. **啟用 WAL 模式**：
   在資料庫初始化時，必須設定 `PRAGMA journal_mode=WAL;`。
2. **限制單一連線**：
   GORM 連線池必須限制為單一連線：
   ```go
   sqlDB, _ := db.DB()
   ...
   ```
3. **單一寫入者 (Worker Pattern) 與 批次寫入 (Batch Insert)**：
   - 非同步任務若有大批量資料寫入時，必須遵循**批量寫入原則**。程式應先在記憶體中收集好一批資料（例如 500 筆為一個批次）。
   - 實作基於 Channel 的單一寫入者模式。非同步任務將收集好的**整批資料**送入 Channel，由專屬的 Worker 進行循序且批量的寫入，這能大幅提升效能並徹底解決 SQLite Locked 問題。
   - **單一交易 (Transaction)**：在 Worker 中，使用單一交易將批次寫入包裝起來，可以大幅減少 SQLite 的磁碟 I/O 次數，進一步將效能極大化。

   ```go
   type ImportBatchTask struct {
       TaskID     string       // 關聯的任務 ID
       DataBatch  []MyModel    // 批次資料
       ResultChan chan error   // 回報寫入結果給 TaskManager
   }

   // 建立一個有緩衝的 Channel
   var writeQueue = make(chan ImportBatchTask, 50)

   // 在程式啟動時，啟動單一 Worker
   func StartDatabaseWriter(db *gorm.DB) {
       go func() {
           for task := range writeQueue {
               // 使用單一交易 (Transaction) 包裝批次寫入
               err := db.Transaction(func(tx *gorm.DB) error {
                   // 所有寫入都在這裡「循序」且「批量」執行
                   // 效能比非同步逐筆寫入快上百倍，且絕對不會發生 SQLite Locked
   ...
   // 非同步任務收集好資料後，把整批資料丟進 Channel
   func HandleAsyncWork(taskID string) {
       go func() {
           // 收集資料
           var records []MyModel
           for _, row := range excelRows {
               records = append(records, parse(row))
           }
           ...
           // 送出批量寫入任務
           writeQueue <- ImportBatchTask{
               TaskID:     taskID,
               DataBatch:  records,
               ResultChan: resultChan,
           }
           
           // 阻塞等待寫入結果，確保任務狀態能正確更新
           if err := <-resultChan; err != nil {
               // 將錯誤回報給 TaskManager，中斷任務
               // UpdateTaskStatus(taskID, TaskFailed, err)
               return
           }
           
           // 寫入成功，繼續下一步或完成任務
           // UpdateTaskStatus(taskID, TaskCompleted, nil)
       }()
   }
   ```
---
...
為確保前端渲染（特別是 Vue DataTable）具備穩定的欄位結構，後端回傳資料時應遵循以下 DTO 定義：

```go
// SecondSourceDTO 替代料 DTO
...
#### 5.1.5 任務與 SQLite Single Writer 的整合防呆

當非同步任務涉及資料庫批量寫入（如 3.3.3 所述）時，**絕對不能**直接將資料丟入 fire-and-forget 的 channel 就視為任務完成。必須透過包含 `TaskID` 與 `ResultChan` 的結構體傳遞，任務 Goroutine 應阻塞等待 `ResultChan` 回傳的 error。這樣任務管理器才能正確捕捉寫入錯誤，將 `TaskStatus` 更新為 Failed，避免進度條永遠卡在 Running，或是匯入失敗卻在前端顯示成功。
...
1. **辨識格式**：對每個選擇的檔案進行格式辨識。若格式判定為 `Unknown` 或 `Matrix`（目前暫不支援 Matrix 匯入），則將該檔案名稱記錄於排除清單中, 記錄於日誌並直接 **Bypass**（跳過）。
2. **啟動非同步任務**：針對所有成功識別且支援格式的檔案，系統會分別為其建立非同步任務，並啟動對應格式的 Reader。
3. **錯誤提示彈窗**：當所有有效檔案都已順利創建任務後，若排除清單中存在被 Bypass 的 `Unknown` 或 `Matrix` 格式檔案，系統會以**彈窗（Popup）通知使用者**哪些檔案因格式無法辨識或暫不支援而未被匯入。
...
1. **選擇格式與來源**：使用者選擇匯出格式（BigMatrix 或 Matrix），並勾選一個或多個要匯出的 BOM Revisions。
   ...
   - **BigMatrix Model 數量調整**：
     1. 若選擇 BigMatrix 匯出，介面需提供選項供使用者調整（增減）每個 BOM 表所要匯出的 Model 數量。預設值為該 BOM 已存在的最長 Model 數量，若該BOM無 Model, 預設為 3。若使用者將數量調大（例如：原本只有 3 個 Model，選擇匯出 4 個），則第 4 個 Model 會按照範本格式繪製，但其內容格留空。
     2. 介面提供一個可輸入description的欄位, 選填。匯出的BigMatrix的 description欄位會填入使用者輸入的description。
   - **Matrix Model 數量**：
     1. 若選擇 Matrix 匯出，僅匯出該 BOM 表中存在 Model 數量，無可調整之功能。若該BOM無包含 Model 資料，則一樣執行匯出，只是 Model 部分資料為空。   
2. **啟動非同步任務與處理策略**：
...
當匯入的 BOM（以 `project`, `phase`, `version` 作為唯一識別）已經存在於資料庫中時，系統依據**來源格式**採用不同的更新策略：

#### 7.0.1 EBOM 匯入：差異合併 (Merge) 策略

EBOM 匯入的核心原則是**更新物料資料的同時，盡可能保留既有的 Matrix 勾選狀態**。各資料層級的處理方式如下：

| 資料層級 | 策略 | 說明 |
|----------|------|------|
| BomRevision 元資料 | **覆蓋更新** | 表頭解析的欄位直接以新值更新 |
| Parts（原子化零件） | **刪除重建** | 刪除該 Revision 所有舊 Parts，重新插入新資料 |
| SecondSources（替代料） | **差異合併** | 以群組鍵比對，增刪改個別 SecondSource |
| MatrixSelections（勾選） | **條件清理** | 僅刪除「被選中物料已不存在」的勾選，其餘保留 |
| MatrixModels | **不影響** | EBOM 匯入不增刪 Model |

> 詳細 Merge 演算法請參閱 **7.1.7**。

#### 7.0.2 BigMatrix 匯入：勾選全覆蓋策略

BigMatrix 本身包含完整的 Model 勾選狀態，匯入時採用**清除該 Revision 所有舊的 MatrixSelections 後重新插入**的做法。BigMatrix 匯入不建立新物料，僅更新 Matrix 勾選狀態與 Model qty。
...
### 7.1 EBOM 匯入規則
EBOM 匯入時，在 database 建立或更新相對應的 BOM revision 資料以及零件資訊。若該 BOM Revision 已存在，則依 7.0.1 的差異合併策略執行更新。
...
- 若 `item` **有值**（或有明確的項次編號）→ **Main Source**
- 若 `item` 為空且緊跟在 Main Source 之後 → **2nd Source**，歸屬於上一個 Main Source 群組
...
當匯入的 EBOM 對應的 BOM Revision 已存在於資料庫中時，系統須執行以下差異合併流程。所有步驟應在**單一資料庫交易 (Transaction)** 中完成，以確保原子性。

##### 步驟 1：更新 BomRevision 元資料

以新 Excel 表頭解析出的欄位（`description`、`schematic_ver`、`pcb_ver` 等）直接覆蓋更新既有的 `BomRevision` 記錄。同時更新 `source_file`、`updated_at` 等欄位。

##### 步驟 2：重建 Parts（原子化零件）

1. 刪除該 Revision 下所有舊的 `Part` 記錄
2. 將新 EBOM 解析出的所有原子化零件批次插入

> Parts 的重建不影響 Matrix 勾選，因為 `MatrixSelection` 是透過 `(main_supplier, main_supplier_pn)` 群組鍵關聯，而非直接關聯 Part ID。

##### 步驟 3：差異合併 SecondSources（替代料）

以 `(revision_id, main_supplier, main_supplier_pn)` 為**群組鍵**，對每個物料群組執行差異比對。個別 SecondSource 的唯一識別為 `(supplier, supplier_pn)`。

1. **從資料庫載入**該 Revision 所有舊的 SecondSource，按群組鍵分組
2. **從新 EBOM 解析**所有 SecondSource，按群組鍵分組
3. **逐群組比對**：

| 情境 | 處理方式 |
|------|----------|
| 舊群組存在、新群組也存在 | 進入子層級 Diff（見下方） |
| 舊群組存在、新群組不存在 | 刪除該群組所有 SecondSource |
| 舊群組不存在、新群組存在 | 批次插入該群組所有 SecondSource |

4. **子層級 Diff**（同一群組內的 SecondSource 增刪改）：
   - 以 `(supplier, supplier_pn)` 為識別鍵
   - **僅存在於舊資料** → 刪除該筆 SecondSource
   - **僅存在於新資料** → 插入該筆 SecondSource
   - **兩邊都存在** → 更新屬性（`hhpn`、`description` 等）

##### 步驟 4：清理無效的 MatrixSelections

根據步驟 3 的差異結果，清理已失效的勾選紀錄：

1. **群組被移除**：若某群組 `(main_supplier, main_supplier_pn)` 在新 EBOM 中已消失，則刪除該群組下所有 `MatrixSelection`
2. **被選中的物料被移除**：對仍存在的群組，檢查每一筆 `MatrixSelection` 的 `(selected_supplier, selected_supplier_pn)` 是否仍存在於該群組的物料集合（主料 + 所有 2nd Source）中。若該物料已不存在，則**刪除該筆 MatrixSelection**（勾選歸空）
3. **被選中的物料仍存在**：不做任何變動，勾選保留

##### 範例驗證

```
【舊資料】
  群組鍵: (SupplierX, PN-001)  ← 主料 P1
  2nd Sources: P2(SupplierY, PN-002), P3(SupplierZ, PN-003)
  MatrixSelections:
    Model A → P1 (SupplierX, PN-001)
    Model B → P2 (SupplierY, PN-002)
    Model C → P3 (SupplierZ, PN-003)

【新 EBOM】
  群組鍵: (SupplierX, PN-001)  ← 主料 P1（不變）
  2nd Sources: P3(SupplierZ, PN-003), P4(SupplierW, PN-004)
...
【步驟 4 結果】
  Model A → P1：P1 仍為主料 → 保留 ✅
  Model B → P2：P2 已被刪除 → 刪除此勾選（歸空）✅
  Model C → P3：P3 仍存在   → 保留 ✅
...
- 若 `item` **有值**（或有明確的項次編號）→ **Main Source**
- 若 `item` 為空且緊跟在 Main Source 之後 → **2nd Source**，歸屬於上一個讀取到的 Main Source 群組。
...
- BigMatrix 匯入時不建立新的物料，僅更新資料庫中每份 BOM 的 Matrix 勾選狀態以及每個 Model 的 qty。
- 若有增減 Model 數量，則同步於資料庫中調整。
- **例外處理**：若解析到的 BOM Revision 在資料庫中不存在，系統應在日誌中記錄 `WARN` 級別的錯誤，並略過該份 BOM 的 Matrix 狀態更新。
...
type MatrixSelection struct {
    ID              string    `gorm:"primaryKey"`
    RevisionID      string    `gorm:"index:idx_sel_revision"`
